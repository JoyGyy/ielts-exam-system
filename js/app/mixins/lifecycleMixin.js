...
const lifecycleMixin = {
    ...
    async loadInitialData() {
        try {
            ...
            // 加载浏览过滤器状态
            const browseFilter = await window.storage.get('browse_filter', { category: 'all', type: 'all' });
     ...
     */
    async loadUserStats() {
        const stats = await window.storage.get('user_stats', {
            totalPractices: 0,
            totalTimeSpent: 0,
            averageScore: 0,
            categoryStats: {},
    ...
    updateCategoryStats(examIndex, practiceRecords) {
        ...
        categories.forEach(category => {
            ...
            const categoryRecords = practiceRecords.filter(record => {
                const exam = window.getExamById ? window.getExamById(record.examId) : list.find(e => e.id === record.examId);
    ...
    renderOverviewCards(examIndex) {
        ...
        console.log('[OverviewCards] stats 计算完成, reading:', stats.reading?.length, 'listening:', stats.listening?.length, 'specialListening:', stats.specialListening?.length);
        const app = this;
...
window.ExamSystemAppMixins.lifecycle = lifecycleMixin;
