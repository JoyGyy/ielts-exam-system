...
const stateMixin = {
    ...
    async persistState(path, storageKey = null) {
        ...
        try {
            ...
            const serializedValue = window.StateSerializer.serialize(value);
            await window.storage.set(key, serializedValue);
        } catch (error) {
    ...
    async loadState(path, storageKey = null) {
        ...
        try {
            const value = await window.storage.get(key, null);
            if (value !== null) {
                const deserializedValue = window.StateSerializer.deserialize(value);
...
window.ExamSystemAppMixins.state = stateMixin;
