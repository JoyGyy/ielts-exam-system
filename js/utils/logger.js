...
class AppLogger {
    constructor(config = {}) {
        this.nativeConsole = { ...window.console }; // Backup native methods
            ...
            if (this.nativeConsole[method]) {
                window.console[method] = (...args) => {
                    ...
                    if (categoryInfo) {
                        ...
                        this.nativeConsole[method].apply(window.console, args);
                    }
                };
            }
        });
    ...
    output(category, level, args) {
        ...
        consoleMethod.call(window.console, prefix, ...args);
    }
    ...
    internalLog(level, message, data) {
        const consoleMethod = this.nativeConsole[level] || this.nativeConsole.log;
        if (data) {
            consoleMethod.call(window.console, `[Logger] ${message}`, data);
        } else {
            consoleMethod.call(window.console, `[Logger] ${message}`);
        }
    }
...
const appLogger = new AppLogger(typeof window !== 'undefined' ? (window.__APP_LOG_CONFIG || {}) : {});
