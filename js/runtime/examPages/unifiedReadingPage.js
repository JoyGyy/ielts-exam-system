(function initUnifiedReadingPage(global) {
    ...
    function parseQuery() {
        ...
        const queryFlowMode = decodeParam(params.get('flowMode')).trim().toLowerCase();
        if (queryFlowMode === 'simulation') {
            const rawIndex = Number(params.get('sequenceIndex'));
            const rawTotal = Number(params.get('sequenceTotal'));
    ...
    function buildEnvelope(type, payload) {
        return {
            ...
            data: Object.assign({
                ...
                globalTimerAnchorMs: state.simulationGlobalAnchorMs,
    ...
    function buildInitSignature(data = {}) {
        return JSON.stringify({
            ...
            flowMode: data && typeof data.flowMode === 'string' ? data.flowMode.trim().toLowerCase() : '',
    ...
    function getSimulationDraftStorageKey() {
        const sessionId = state.sessionId ? String(state.sessionId).trim() : '';
        const examId = state.examId ? String(state.examId).trim() : '';
        if (!sessionId || !examId) {
            return '';
        }
        return `ielts_sim_draft::${sessionId}::${examId}`;
    }
    ...
    function syncSimulationDraftSnapshot(reason = 'periodic') {
        if (!state.simulationMode || state.readOnly || !state.sessionId) {
            return;
        }
    ...
    function handleExitClick() {
        const inSuiteLikeMode = Boolean(state.reviewMode);
    ...
    function handleIncoming(event) {
        ...
        if (type === 'INIT_SESSION' || type === 'INIT_EXAM_SESSION') {
            ...
            const initFlowMode = data && typeof data.flowMode === 'string'
                ? data.flowMode.trim().toLowerCase()
                : '';
            if (initFlowMode === 'simulation') {
                const rawIndex = Number(data.sequenceIndex);
                const rawTotal = Number(data.sequenceTotal);
                const currentIndex = Number.isFinite(rawIndex) ? Math.max(0, rawIndex) : 0;
                const total = Number.isFinite(rawTotal) && rawTotal > 0 ? rawTotal : 3;
                const isLast = currentIndex >= total - 1;
                state.simulationMode = true;
        ...
        if (type === 'SIMULATION_CONTEXT') {
            ...
            const simulationTimerAnchorMs = Number(data.globalTimerAnchorMs);
            ...
            const snapshotRunning = timerSnapshot ? timerSnapshot.running : true;
